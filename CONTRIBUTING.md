# Contributing to MCP Browser Use Server

First off, thank you for considering contributing to MCP Browser Use Server!
...
2. Clone your fork:
  `git clone https://github.com/your-username/mcp-browser-use-server.git`
...
- Search existing
  [Issues](https://github.com/hubertusgbecker/mcp-browser-use-server/issues) to
...
By contributing to MCP Browser Use Server, you agree that your contributions
will be licensed under the MIT License. See [LICENSE](LICENSE) for details.

## Git Hooks (pre-commit)

To keep the codebase lint-clean, we provide a Git pre-commit hook that runs
`uv run ruff check .` before committing. Install the hooks by running from
the repository root:
...
This will copy the hooks from `.githooks/` into `.git/hooks/`. If `uv` is not
available on your PATH the hook will warn and skip the check.
